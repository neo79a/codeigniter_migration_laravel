...
class CI_Encryption {
    ...
    function get_key($key = '')
    {
        if ($key == '')
        {
            ...
            if ($key == FALSE)
            {
                logger('In order to use the encryption class requires that you set an encryption key in your config file.');
            }
        }
    ...
    function encode_from_legacy($string, $legacy_mode = MCRYPT_MODE_ECB, $key = '')
    {
        ...
        {
            logger('error', 'Encoding from legacy is available only when Mcrypt is in use.');
            return FALSE;
        }
